...
def main(preprocessor_path, train_data_path, model_save_path, table_dir, plot_dir):
    ...
    best_model_scores_svc.to_csv(os.path.join(table_dir, "cv", "rf_cv.csv"))
    accuracies_grid = model.get_accuracy_grid()
    accuracies_grid.to_csv(os.path.join(table_dir, "rf.csv"))
    plot = model.draw_search_plot_2()
    plot.save(os.path.join(plot_dir, "rf.png"))
