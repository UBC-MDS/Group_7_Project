...
def main(preprocessor_path, train_data_path, model_save_path, table_dir, plot_dir):
    ...
    best_model_scores_svc.to_csv(os.path.join(table_dir, "cv", "knn_cv.csv"))
    accuracies_grid = model.get_accuracy_grid()
    accuracies_grid.to_csv(os.path.join(table_dir, "knn.csv"))
    plot = model.draw_search_plot()
    plot.save(os.path.join(plot_dir, "knn.png"))
