...
-  Open up a terminal in the Jupyter IDE of the Docker container, and use the terminal in the project root of the Docker container to navigate to the `work` directory using the command: `cd ./work`

-  Use the command `make clean` in the terminal of the Docker container to remove all the files previously associated with this report - in other words, 'resetting' the project and allowing you to run it for yourself from scratch.
...
4.  Then, use the command `make all` to re-run our analyses and regenerate our report. Our final rendered report html file should be located at `/docs/bank_marketing_prediction.html`.
